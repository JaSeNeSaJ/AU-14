...
public sealed partial class XenoHudOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        var isBiomorph = _entity.HasComponent<BiomorphComponent>(_players.LocalEntity) ||
                         _entity.HasComponent<BiomorphMimicTransformedComponent>(_players.LocalEntity);
        var isGhost = false;

        if (!_entity.HasComponent<CMGhostXenoHudComponent>(_players.LocalEntity))
        {
            if (!isXeno && !isAdminGhost && !isBiomorph)
                return;
        }
        else
        {
