...
public sealed class HumanSurgerySystemTest
{
    ...
    [Test]
    public void HealedRepairTargetRemapsCommittedOperationToSurgicalAccess()
    {
        var medical = HumanMedicalLedger.CreateDefault();
        ...
        var changed = HumanSurgerySystem.RefreshActiveOperations(medical, operations);
        var close = Attempt(
            BodyRegion.Groin,
            SurgeryStepKind.CloseIncision,
            procedureId: SurgeryProcedureId.SurgicalAccess);
        var canClose = HumanSurgeryProcedureRules.TryReserveOperation(operations, close, out var failure);

        Assert.Multiple(() =>
        {
            Assert.That(changed, Is.True);
            Assert.That(canClose, Is.True, failure);
            Assert.That(operations, Has.Count.EqualTo(1));
            Assert.That(operations[0].ProcedureId, Is.EqualTo(SurgeryProcedureId.SurgicalAccess));
            Assert.That(operations[0].Committed, Is.True);
        });
    }

    [Test]
    public void HealedRepairTargetRemapsCommittedOperationToNextRequiredProcedure()
    {
        var medical = HumanMedicalLedger.CreateDefault();
        DamageOrgan(medical, OrganSlot.Kidneys, FixedPoint2.New(30));
        BreakRegion(medical, BodyRegion.Groin);
        OpenToShallowAccess(medical, BodyRegion.Groin);
        var operations = new List<SurgeryOperationState>();
        var open = Attempt(
            BodyRegion.Groin,
        ...
        Assert.Multiple(() =>
        {
            Assert.That(changed, Is.True);
            Assert.That(operations, Has.Count.EqualTo(1));
            Assert.That(operations[0].ProcedureId, Is.EqualTo(SurgeryProcedureId.RepairFracture));
            Assert.That(operations[0].Committed, Is.True);
        });
    }

    [Test]
    public void CurrentRepairTargetKeepsCommittedOperation()
    {
        var medical = HumanMedicalLedger.CreateDefault();
        DamageOrgan(medical, OrganSlot.Kidneys, FixedPoint2.New(30));
        OpenToShallowAccess(medical, BodyRegion.Groin);
    ...
    [Test]
    public void ClosedIncisionClearsCommittedRepairOperation()
    {
        var medical = HumanMedicalLedger.CreateDefault();
        ...
        var close = HumanSurgerySystem.TryApplySurgery(
            medical,
        ...
        var hasAccess = HumanSurgeryProcedureRules.HasOpenSurgicalAccess(medical, BodyRegion.LeftArm);

        var changed = HumanSurgerySystem.RefreshActiveOperations(medical, operations);

        Assert.Multiple(() =>
        {
            Assert.That(close.Applied, Is.True, close.FailureReason);
