...
public sealed partial class CMUExplosionMedicalTraumaSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<HumanMedicalComponent, ExplosionReceivedEvent>(
            OnExplosionReceived,
            before: [typeof(SharedRMCExplosionSystem)]);
    }

    private void OnExplosionReceived(Entity<HumanMedicalComponent> ent, ref ExplosionReceivedEvent args)
    {
        ...
        if (args.Damage.GetTotal() > FixedPoint2.Zero)
        {
            ...
            foreach (var weighted in weightedParts)
            {
                ...
                ApplyLedgerExplosionDamage(ent, weighted, args.Damage * scale);
            }
        }
    ...
    private void ApplyLedgerExplosionDamage(
        Entity<HumanMedicalComponent> body,
        SharedCMUShrapnelSystem.WeightedBodyPart weighted,
        DamageSpecifier damage)
    {
        var region = ResolveExplosionRegion(weighted);
        ...
        var currentRegionState = HumanMedicalLedger.GetRegion(body.Comp, region);
        ...
        RaiseLocalEvent(body.Owner, ref ev, broadcast: true);
    }
    ...
    private BodyRegion ResolveExplosionRegion(SharedCMUShrapnelSystem.WeightedBodyPart weighted)
    {
        ...
        return weighted.Type switch
        {
            BodyPartType.Head => BodyRegion.Head,
            BodyPartType.Torso => BodyRegion.Chest,
            BodyPartType.Arm => weighted.Symmetry == BodyPartSymmetry.Right ? BodyRegion.RightArm : BodyRegion.LeftArm,
            BodyPartType.Hand => weighted.Symmetry == BodyPartSymmetry.Right ? BodyRegion.RightHand : BodyRegion.LeftHand,
            BodyPartType.Leg => weighted.Symmetry == BodyPartSymmetry.Right ? BodyRegion.RightLeg : BodyRegion.LeftLeg,
            BodyPartType.Foot => weighted.Symmetry == BodyPartSymmetry.Right ? BodyRegion.RightFoot : BodyRegion.LeftFoot,
            _ => BodyRegion.None,
        };
    }
